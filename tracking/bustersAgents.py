...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for belief in livingGhostPositionDistributions:
            locMax.append(belief.argMax())
        goalCoord = None
        goalProb = 0
        
        for index, coord in enumerate(locMax):
        ...
        choose = []
        for action in legal:
            nextLoc = Actions.getSuccessor(pacmanPosition, action)
            choose.append((self.distancer.getDistance(nextLoc,goalCoord), action))
        return min(choose)[1]